...
describe("a game against a house player", () => {
  // The human is rated; the house player is rated AGAINST and never rated.
  // Its `users.rating` is a fixed number the platform chose, and a bot whose
  // rating drifted with its results would not be the opponent its card says.

  it("white bot: only black's row moves, and it moves the right way", async () => {
    const lost = await run(
      { ...BASE, white: { rating: 1500, isHouseBot: true }, black: { rating: 1200 } },
      { result: GameResult.WHITE_WIN, userId: "black-user" }
    );
    expect(lost.calls.ratingReads).toEqual(["black-user"]);
    expect(lost.calls.ratingUpserts.map((u) => (u as { where: { userId: string } }).where.userId)).toEqual([
      "black-user",
    ]);
    expect(lost.calls.userUpdates.map((u) => (u as { where: { id: string } }).where.id)).toEqual(["black-user"]);
    expect(lost.calls.transactions).toBe(1);
    const afterLoss = (lost.calls.userUpdates[0] as { data: { rating: number } }).data.rating;
    expect(afterLoss).toBeLessThan(1200);

    const won = await run(
      { ...BASE, white: { rating: 1500, isHouseBot: true }, black: { rating: 1200 } },
      { result: GameResult.BLACK_WIN, userId: "black-user" }
    );
    const afterWin = (won.calls.userUpdates[0] as { data: { rating: number } }).data.rating;
    expect(afterWin).toBeGreaterThan(1200);
  });

  it("black bot: only white's row moves", async () => {
    const { calls } = await run(
      { ...BASE, white: { rating: 1200 }, black: { rating: 900, isHouseBot: true } },
      { result: GameResult.WHITE_WIN }
    );
    expect(calls.ratingReads).toEqual(["white-user"]);
    expect(calls.userUpdates.map((u) => (u as { where: { id: string } }).where.id)).toEqual(["white-user"]);
    expect((calls.userUpdates[0] as { data: { rating: number } }).data.rating).toBeGreaterThan(1200);
  });

  it("two bots rate nobody", async () => {
    const { calls } = await run(
      { ...BASE, white: { rating: 1200, isHouseBot: true }, black: { rating: 900, isHouseBot: true } },
      { result: GameResult.WHITE_WIN }
    );
    expect(ratingWrites(calls)).toBe(0);
  });

  it("without the flag the pair path is untouched", async () => {
    const { calls } = await run(BASE, { result: GameResult.WHITE_WIN });
    expect(calls.ratingReads).toEqual(["white-user", "black-user"]);
    expect(calls.transactions).toBe(1);
  });
});
